...
class IntegralValue:
    ...
    def Calculate(integral):
        ...
        for index1 in range(len(integral.MethodsNames)):
            for index in range(len(Intervals)):
                IntegralValue = integral.MethodsNames[index1](Intervals[index])
                Accuracy = abs(integral.AnalyticValue - IntegralValue)
                print (f"Method {Methods[index1]}, n = {Intervals[index]} :  {IntegralValue} with accuracy : {round(Accuracy,6)} ")
